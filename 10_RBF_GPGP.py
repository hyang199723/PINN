...
dat = np.array(pd.read_csv(wk_dir + "Data/gpgp_02_1_5.csv", index_col=False, header = None))
dat = np.delete(dat, (0), axis=0).astype(float)
size = 1300 * 3
subdat = dat[0:size, :]
original_dimension = (1300, 3, 100)
dat_full = subdat.reshape(original_dimension)
alphas = [2, 4, 8, 16]#,0.01, 0.03, 0.05, 0.08, 0.1, 0.2, 0.3, 0.5, 0.8, 1, 5, 10]
iters = 30
...
MSE.to_csv(wk_dir + "Output_New/gpgp_highV_sup.csv")
# %%
